...
import PayslipGeneratorDialog from "./payslip-management/PayslipGeneratorDialog";
import PayslipEditDialog from "./payslip-management/PayslipEditDialog";
import PayslipStatusDialog from "./payslip-management/PayslipStatusDialog";
import PayslipPreviewModal from "./payslip-management/PayslipPreviewModal";
...
export default function HRPayslipManagement() {
  ...
  const handleDeletePayslip = async (payslip: EmployeePayroll) => {
    // Confirm deletion
    const confirmed = window.confirm(
      `Are you sure you want to delete the payslip for ${payslip.name} (${payslip.payrollPeriod.label})?\n\nThis action cannot be undone.`
    );
    
    if (!confirmed) return;

    // Check if payslip is in draft status
    if (payslip.status !== 'draft') {
      alert('Only draft payslips can be deleted. Please set the status to draft first.');
      return;
    }

    try {
      const response = await axiosInstance.delete(`/payroll/delete/${payslip._id}`);
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        title={payslip.status === 'draft' ? "Delete Payslip" : "Only draft payslips can be deleted"}
                        className={payslip.status === 'draft' ? "text-red-600 hover:text-red-700 hover:bg-red-50" : ""}
                      >
                        <Trash2 size={14} />
                      </Button>
                    </div>
