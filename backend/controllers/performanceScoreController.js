...
const createPerformanceScore = async (req, res) => {
  try {
    ...
    const employee = await User.findOne({ id: performanceScoreData.employeeId })
    if (!employee) {
      return res.status(404).json({
        success: false,
    ...
    await performanceScore.populate("employeeId", "name email department role")
    ...
    if (error.code === 11000) {
      return res.status(400).json({
        success: false,
        error: "Performance score already exists for this employee in this period"
      })
    }
...
const addManagerEvaluation = async (req, res) => {
  try {
    ...
    await performanceScore.save()
    await performanceScore.populate("managerEvaluation.managerId", "name email")
...
const calculateIncrementEligibility = async (req, res) => {
  try {
    ...
    const incrementEligibility = performanceScore.calculateIncrementEligibility(companyGrowth)
...
const getPerformanceAnalytics = async (req, res) => {
  try {
    ...
    const averageScore = totalEmployees > 0 
      ? Math.round(performanceScores.reduce((sum, score) => sum + score.scores.totalScore, 0) / totalEmployees)
      : 0
